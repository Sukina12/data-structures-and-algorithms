...
const notInFirstArray = (forbiddenValues, arr) => {
  let events = arr.filter(currentValue => (forbiddenValues.includes(currentValue) === false));
  return events;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  ...
  let events = arr.filter(currentValue => {
    return (currentValue.baseStat > minBaseStat);
  });
  return events;
};
...
const getStatName = (arr, minBaseStat) => {
  let newArray = arr.filter(value => value.baseStat > minBaseStat).map(value => value.stat.name);
  return newArray;
};
...
const getCharactersWithoutChildren = (arr) => {
  let newArray = arr.filter(value => !value.children);
  return newArray;
};
...
const evenOddNumericValues = (arr) => {
  let newArray = arr.filter(value => typeof value === 'number').map(value => value % 2 === 0 ? 'even' : 'odd');
  return newArray;
};
