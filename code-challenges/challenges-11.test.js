...
const divisibleByFiveTwoToThePower = (input) => {
  let result = input.map(element => {
    let filterData = element.filter(number => {
...
let findMaleAndFemale = (data) => {
  let filterData = data.filter (items => {
    return items.gender === 'male' || items.gender === 'female';
  }).map (items => {
    return items.name;
  }).join (' and ');
  return filterData;
...
let findShortest = (data) => {
  let newArray = [];
  let shortest = data.map (items => {
    let shortestPerson = items.name.match (/([A-z])/g);
    newArray.push(shortestPerson.length);
    return items.name;
  });
